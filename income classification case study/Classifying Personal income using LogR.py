...
sns.heatmap(cf_matrix, cmap='YlGnBu', annot=True,fmt="d")
...
pred_prob_lmod1 = logistic.predict_proba(test_x)[:,1]


# ROC Curve
f, ax = plt.subplots(figsize=(5,4))
fpr, tpr, thresholds = roc_curve(test_y,pred_prob_lmod1)
plt.plot([0, 1], [0, 1], linestyle='--')
# ROC Curve 
plt.plot(fpr, tpr, marker='.',label='AUC = {:.2f}'.format(auc(fpr, tpr)))
ax.set_title('ROC Curve ' +'Logistic'+'\nAUC ROC Score:{0:.4f}'.format(roc_auc_score(test_y,prediction)))
plt.xlabel("False Positive Rate")
plt.ylabel("TruePositive Rate")
plt.show()
    

# ROC_Curve(test_y, pred_prob_log_y,pred_log_y,'Logistic')
